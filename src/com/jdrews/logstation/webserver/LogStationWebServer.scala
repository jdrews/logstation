...
you'd need to either run a long poll or a websocket

with an event handler that appends the DOM and adjusts the scroll when data is received

their would of course need to be a flag set for whether or not to autoscroll

that is on by default, but can be turned off with a scroll up event

and then turned on when scrolled to bottom again

 Jon:  does spray can handle websockets?
the only problem with websockets is browser backward compatibility
now that I think about it, long polling is probably what you want to do
have you looked into comet at all?

 */
class LogStationWebServer extends Actor with ActorLogging {
...
//            }
        ...
        case lm: LogMessage =>
            log.info(s"got log message $lm")
            sender ! HttpResponse(entity = s"${lm.logFile}, ${lm.logMessage}")
